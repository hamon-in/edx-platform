...
class TestCourseHomePageAccess(CourseHomePageTestCase):
    ...
    def test_first_purchase_offer_banner_display(self,
        ...
        url = course_home_url(self.course)
        response = self.client.get(url)
        discount_expiration_date = get_discount_expiration_date(user, self.course).strftime('%B %d')
        ...
        bannerText = u'''<div class="first-purchase-offer-banner">
             ...
             <br>Discount will be automatically applied at checkout. <a href="{upgrade_link}">Upgrade Now</a>
                ...
                strikeout_price=HTML(format_strikeout_price(user, self.course, already_checked_discount=True)[0]),
