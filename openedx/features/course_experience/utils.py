...
def get_first_purchase_offer_banner_fragment(user, course):
    if user and course:
        ...
        if (discount_expiration_date and
            ...
            offer_message = _(u'{banner_open} Upgrade by {discount_expiration_date} and save {percentage}% [{strikeout_price}]{span_close}'
                              u'{br}Discount will be automatically applied at checkout. {a_open}Upgrade Now{a_close}{div_close}')
            return Fragment(HTML(offer_message).format(
                ...
                discount_expiration_date=discount_expiration_date.strftime('%B %d'),
                percentage=discount_percentage(),
                span_close=HTML('</span>'),
                div_close=HTML('</div>'),
                strikeout_price=HTML(format_strikeout_price(user, course, already_checked_discount=True)[0])
            ))
    return None
