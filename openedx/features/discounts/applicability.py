...
def get_discount_expiration_date(user, course):
    course_enrollment = CourseEnrollment.objects.filter(user=user, course=course.id, mode__in=CourseMode.UPSELL_TO_VERIFIED_MODES)
...
def can_receive_discount(user, course, discount_expiration_date=None):  # pylint: disable=unused-argument
    """
